...
export default function ImpossibleCheckboxFaithful() {
  ...
  // Random limits exactly like original
  const [armLimit] = useState(() => Math.random() * 3)
  const [headLimit] = useState(() => armLimit + 1 + Math.random() * 2)
  const [angerLimit] = useState(() => headLimit + 1 + Math.random() * 2)

  // Durations exactly like original
  ...
  if (SOUNDS.GROAN) SOUNDS.GROAN.playbackRate = 2

  useEffect(() => {
    loadGSAP().then(setGsap)
  }, [])

  // Exact hover handlers from original
  const onHover = () => {
    if (!gsap) return
    if (Math.random() > 0.5 && count > armLimit) {
      gsap.to(bearRef.current, { duration: bearDuration / 2, y: "40%" })
    }
  }
  ...
  const offHover = () => {
    if (!gsap) return
    if (!checked) {
      gsap.to(bearRef.current, { duration: bearDuration / 2, y: "100%" })
    }
  }
  ...
  // Exact useEffect logic from original
  useEffect(() => {
    if (!gsap || !checked) return
  ...
  // Set initial bear position
  useEffect(() => {
    if (gsap && bearRef.current) {
      gsap.set(bearRef.current, { y: "100%" })
    }
  }, [gsap])

  if (!gsap) return <div>Loading...</div>
